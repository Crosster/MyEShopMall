...
@WebServlet(urlPatterns = "/ShowProductByName.do")
public class ShowProductByName extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		String pnameStr = request.getParameter("search");
		try {
			Connection conn = MyConnectionFactory.getConnection();
			ProductDAO productDAO = new ProductDAO(conn);
			List<Product> productList = productDAO.findProductByName(pnameStr);
